...
assert.ok(!/Reviewer/.test(quizFlow), 'quiz flow hiển thị gọn không cần Reviewer')
...
assert.match(runners, /hình lập phương\|hinh lap phuong/)
assert.match(runners, /trạng thái của chất\|trang thai cua chat/)
