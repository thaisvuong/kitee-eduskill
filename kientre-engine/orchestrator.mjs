...
export async function composeDocument(topic, grade, subject = 'Toán', opts = {}) {
 try {
  ...
  const visualModel = process.env.HERMES_VISUAL_MODEL || drafter
  ...
  const directReference = String(opts.refs || '').trim()
  ...
  if (useWeb) {
   ...
   console.log('📝 Không có tài liệu ngoài. Tự soạn theo chuyên đề đã yêu cầu.')
  }
  if (opts.notebook) {
   try {
  ...
  const draftRefs = [directReference, notebookReference, webReference].filter(Boolean).join('\n\n')
  const exerciseRef = [directReference, notebookReference, webReference].filter(Boolean).join('\n\n')
