...
"""
Kientre_Template.py
Tạo file Word template đơn giản, chỉnh chu cho phiếu học/giáo trình Kientre.
...
Cách chạy:
  ...
  python Kientre_Template.py

File xuất ra:
  Kientre_Template.docx
"""
...
# =========================
OUTPUT_FILE = "/mnt/data/Kientre_Template.docx"
...
def setup_document():
  ...
  if "KientreCaption" not in [s.name for s in doc.styles]:
    st = doc.styles.add_style("KientreCaption", WD_STYLE_TYPE.PARAGRAPH)
    st.font.name = FONT_HEAD
    st.font.size = Pt(9)
...
def add_header_footer(doc):
  ...
  header.text = ""
  r = header.add_run("Kientre Education | Phiếu học")
  ...
  footer.text = ""
  r = footer.add_run("Kientre_Template | Dùng để tạo giáo trình, phiếu học, tài liệu ôn tập")
...
def add_document_title(doc, title, subtitle=""):
  ...
  r = p.add_run("KIENTRE WORD TEMPLATE")
