...
// ponytail: chỉ cho sửa file trong engineDir và hermesHome/skills. add per-user ACL when multi-tenant.
function allowedRoots(): string[] {
 return [
  path.resolve(kientreConfig.engineDir),
  path.resolve(path.join(kientreConfig.hermesHome, 'skills')),
  path.resolve(process.env.KIENTRE_ENGINE_DIR || kientreConfig.engineDir),
 ]
}
...
export async function GET(req: Request) {
 ...
 if (!isInsideAllowed(p)) return NextResponse.json({ ok: false, error: 'Không được phép mở file ngoài Kientre/skills' }, { status: 403 })
...
export async function PUT(req: Request) {
 ...
 if (!p) return NextResponse.json({ ok: false, error: 'Thiếu path' }, { status: 400 })
 if (!isInsideAllowed(p)) return NextResponse.json({ ok: false, error: 'Không được phép ghi file ngoài Kientre/skills' }, { status: 403 })
 const ext = path.extname(p).toLowerCase()
 if (!EDITABLE_EXT.has(ext)) return NextResponse.json({ ok: false, error: `Không sửa được định dạng ${ext}` }, { status: 400 })
