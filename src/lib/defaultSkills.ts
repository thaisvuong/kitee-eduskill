...
export const DEFAULT_SKILLS: SkillDef[] = [
 ...
 { id: 'slash-es-quiz', name: '/es-quiz', description: 'Soạn quiz theo chuyên đề bằng flow QuizPlanner → Examiner từng câu → Artist', systemPrompt: 'Quy trình quiz là pipeline riêng, không dùng pipeline soạn chuyên đề. Không viết lý thuyết, không tạo ví dụ hàng loạt. Bắt buộc sub-agent: Intent đọc yêu cầu → Source/WebSearch lấy nguồn tài liệu/bài tập thật nếu không có NotebookLM → QuizPlanner dùng model mạnh lập bảng khung từng quiz/từng câu/điểm/loại câu/note/hình → Examiner dùng model trung bình-khá soạn từng câu riêng lẻ với context chung → Artist vẽ hoặc lấy hình minh hoạ phù hợp cho câu; ảnh thật phải ưu tiên nguồn web/Openverse/Wikimedia, không tự tạo ảnh trừ khi bật KIENTRE_ALLOW_IMAGE_GENERATION → Student ước lượng thời gian/độ khó → Judge kiểm ranh giới lớp, đáp án, tổng điểm → Reviewer rà lỗi → Word xuất file. Quiz cuối phải cực khó, ít học sinh làm được nhưng không vượt lớp. Mỗi quiz theo thứ tự: đề, gợi ý, đáp án đúng, lời giải chi tiết.', guidance: 'Dùng module quiz với số quiz/tổng điểm/thời gian trên composer. Tool run_skill phải dùng skill="quiz" và truyền quizCount/totalScore/timeMinutes.', appliesTo: ['quiz'], agentFlow: SKILL_FLOWS.quiz, enabled: true },
