#!/usr/bin/env bash
# KientreAAA — cài đặt "all-in-one" cho người dùng cuối.
...
# ── 3) Kientre (đi kèm repo) ───────────────────────────────
REPO_DIR="$(cd "$(dirname "$0")" && pwd)"
KIENTRE_ENGINE_DIR="$REPO_DIR/kientre-engine"
if [ ! -f "$KIENTRE_ENGINE_DIR/slash.mjs" ]; then
 fail "Không tìm thấy kientre-engine/slash.mjs trong repo. Repo có thể bị hỏng — hãy clone lại."
fi
ok "Kientre: $KIENTRE_ENGINE_DIR"

# ── 3b) Python deps cho Kientre ───────────────────────────────────
ensure_python_deps() {
 local py
 for cand in \
  "${HERMES_PYTHON:-}" \
...
# ── 4) Cài dependencies + build Kientre web app ──────────────────────
...
if [ ! -f .env.local ]; then
 USER_HOME="$HOME"
 cat > .env.local <<ENV
# KientreAAA — cấu hình máy của bạn
KIENTRE_WORKSPACE_DIR=$USER_HOME/Kientre
KIENTRE_OUTPUT_DIR=$USER_HOME/Kientre/Output
KIENTRE_ENGINE_DIR=$KIENTRE_ENGINE_DIR
...
Bước tiếp theo:
 ...
 2. Sau khi lưu key, quay lại đây và nhấn ENTER để mở Kientre web app.

Địa chỉ 9router (cấu hình key): http://localhost:20128
Địa chỉ Kientre (dùng app):    http://localhost:3100
