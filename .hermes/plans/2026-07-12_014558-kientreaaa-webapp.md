# KientreAAA Web App Implementation Plan

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Build Kientre as a hostable web app that exposes Kientre/Hermes-powered education workflows first, then adds UI, job history, Drive upload, and 9Router-connected AI configuration.

**Architecture:** Start by wrapping the existing Kientre CLI/agent flows as a local service from the Kientre workspace. The web app submits jobs (`create`, `test`, `solve`, `review`) to a backend queue, stores all artifacts under `Kientre/Output`, optionally uploads to Kientre Drive, and can use 9Router/Hermes model configuration for agent/model routing. Keep Kientre as the engine; Kientre owns product UI, API, auth, storage, and deployment.

**Tech Stack:** Next.js + TypeScript for the web app, Node.js API routes/server actions for backend orchestration, SQLite for job metadata, child-process wrapper around `/Users/nguyenthaivuong/Desktop/HermesWorkspace/Kientre/slash.mjs`, local filesystem output under `/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre/Output`, Google Workspace script for Drive, 9Router-compatible OpenAI API config via environment variables.

---

## Current Context / Assumptions

- Active workspace: `/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre`.
- Kientre repo: `/Users/nguyenthaivuong/Desktop/HermesWorkspace/Kientre`.
- Kientre slash commands already support `/es-create`, `/es-test`, `/es-solve`, `/es-review` in `slash.mjs`.
- Kientre output root has been adjusted to default to `Kientre/Output` for this profile.
- Kientre Drive folder ID: `18fe276zrUdVAlFOFyKHPtc_8-GpEexKn`.
...
- `create`: create lesson/chuyên đề/worksheet via Kientre compose flow.
- `test`: generate exam + answer key via Kientre examiner flow.
- `solve`: upload/select a document, run solver flow.
- `review`: upload/select a document, run reviewer flow.
- Output browsing: list files in `Kientre/Output`, download generated `.docx`, `.pdf`, `.md`, `.json`.
- Optional Drive upload: upload generated files to Kientre Drive when requested.
...
1. **Local-hosted web UI**: app runs on the Mac, can access Kientre repo, Hermes profile files, local output, and local 9Router.
2. **Cloud-hosted control plane**: web UI hosted publicly, but it calls a local Kientre worker/agent via secure tunnel/webhook because cloud cannot directly access the Mac-local Kientre/Hermes/9Router stack.

Prefer local-hosted first because Kientre depends on local filesystem, Python libs, Google OAuth files, and 9Router local endpoint.
...
├── src/lib/kientre/         # wrapper around Kientre slash.mjs
...
│  └── worker.ts           # sequential runner, no parallel Kientre
├── src/lib/drive/          # optional Google Drive upload wrapper
├── src/lib/config/          # Kientre/Hermes/9Router config
...
Key principle: the web app must not reimplement Kientre logic. It builds commands and calls `node slash.mjs "..."` from the Kientre repo with the right env.
...
**Objective:** Create a Next.js TypeScript app inside Kientre without disturbing existing `Output/` or `zalo-bridge/` files.
...
1. Create a minimal Next.js app structure under `/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre`.
2. Add scripts:
...
## Task 2: Add Kientre runtime config module

**Objective:** Centralize all paths, 9Router settings, Drive settings, and defaults in one config file.

**Files:**
- Create: `src/lib/config/kientre.ts`
- Create: `.env.example`

**Implementation notes:**

`src/lib/config/kientre.ts` should expose:

```ts
export const kientreConfig = {
 workspaceDir: process.env.KIENTRE_WORKSPACE_DIR ?? "/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre",
 outputDir: process.env.KIENTRE_OUTPUT_DIR ?? "/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre/Output",
 engineDir: process.env.KIENTRE_ENGINE_DIR ?? "/Users/nguyenthaivuong/Desktop/HermesWorkspace/Kientre",
...
```bash
KIENTRE_WORKSPACE_DIR=/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre
KIENTRE_ENGINE_DIR=/Users/nguyenthaivuong/Desktop/HermesWorkspace/Kientre
HERMES_HOME=/Users/nguyenthaivuong/.hermes/profiles/cmkitee
KIENTRE_OUTPUT_DIR=/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre/Output
HERMES_DRIVE_PARENT_ID=18fe276zrUdVAlFOFyKHPtc_8-GpEexKn
NINE_ROUTER_BASE_URL=http://localhost:20128/v1
HERMES_WORKER_MODEL=gc/gemini-2.5-flash
KIENTRE_DEFAULT_SUMMARY=1
```

**Verification:**

Add a temporary settings/debug page or unit test that imports `kientreConfig` and verifies the default paths.

---

## Task 3: Implement safe Kientre command builder

**Objective:** Convert structured form inputs into safe Kientre slash commands.

**Files:**
- Create: `src/lib/kientre/commands.ts`
- Create: `src/lib/kientre/commands.test.ts` or equivalent test target

**Types:**

```ts
export type KientreMode = "create" | "test" | "solve" | "review";
...
## Task 4: Implement Kientre runner with sequential execution

**Objective:** Run Kientre commands from the web app and stream logs while enforcing one Kientre job at a time.

**Files:**
- Create: `src/lib/kientre/runner.ts`
...
spawn("node", ["slash.mjs", command], {
 cwd: kientreConfig.engineDir,
 env: {
  ...process.env,
  HERMES_HOME: kientreConfig.hermesHome,
  HERMES_WORKSPACE_DIR: kientreConfig.workspaceDir,
  KIENTRE_OUTPUT_DIR: kientreConfig.outputDir,
  HERMES_DRIVE_PARENT_ID: kientreConfig.driveParentId,
  HERMES_WORKER_MODEL: selectedModel ?? kientreConfig.defaultWorkerModel,
  HERMES_UPLOAD_DRIVE: uploadDrive ? "1" : "0"
 }
})
```

Add a simple in-process queue first. Do not run Kientre jobs in parallel because Gemini quota/rate limits are known issues.
...
Expected:
- Job completes.
- Output folder appears under `Kientre/Output`.
- No output is written to `Kientre/output` unless explicitly configured.
...
- Create: `data/kientre.sqlite` at runtime, not committed
...
**Objective:** After a job completes, locate generated artifacts under `Kientre/Output`.

**Files:**
- Create: `src/lib/kientre/output.ts`
...
2. If stdout parsing fails, scan `Kientre/Output` for the newest modified folder matching expected prefix:
  - `G5_...` for create
  - `EXAM_G5_...` for test
  - `SOLVE_G...` for solve
  - `REVIEW_G...` for review
...
```text
/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre/Output/_smoke_profile_output/_smoke_profile_output.docx
```
...
**Objective:** Provide a usable web interface for the four Kientre functions.
...
- Solve/review pages:
 ...
 - later: file upload into `Kientre/uploads`
- Job detail:
 - status
...
- Only serve files under `Kientre/Output` or `Kientre/uploads`.
...
**Objective:** Decide how the web app uses “Hermes functions” beyond Kientre.

**Recommended boundary:**

Start with controlled subprocess calls, not arbitrary full-agent access:

- For Kientre: call `node slash.mjs` directly.
...
**Recommended first:** Cloudflare Tunnel + Cloudflare Access, because the app likely needs to run on the Mac to access Kientre and 9Router.
...
**Objective:** Choose a deployment path that preserves local Kientre/Hermes access.
...
Only viable if:
- Kientre repo and dependencies are installed on server.
...
- Local Kientre worker polls for jobs and executes Kientre locally.
...
```text
/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kientre/Output
```
...
1. **Kientre jobs are slow and quota-bound.** Must run sequentially and expose logs/status.
...
6. **Long-running jobs in serverless environments will timeout.** Avoid Vercel serverless for direct Kientre execution; use local Node server, VPS worker, or queue/worker split.
...
4. Do we want public hosting for external users, or only private access for anh/team Kientre?
...
2. Add config module.
3. Add Kientre command builder.
4. Add sequential runner.
5. Add job DB/API.
6. Prove `/es-test` and `/es-create` generate files under `Kientre/Output` from the web app/API.
