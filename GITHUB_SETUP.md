# 📦 Hướng dẫn đưa Kitee eduSkill lên GitHub & cho người khác dùng
...
```bash
cd /Users/nguyenthaivuong/KiteeApp
...
2. Repository name: `kitee-eduskill` (tùy anh).
...
```bash
gh repo create kitee-eduskill --private --source=. --remote=origin
```
...
git init
git add .
git commit -m "Kitee eduSkill web app - initial release"

git branch -M main
git remote add origin https://github.com/<USERNAME>/kitee-eduskill.git
...
- **eduSkill engine** (thư mục chứa `slash.mjs`) — cần cho tính năng tạo/giải/nhận xét tài liệu.
- (Tùy chọn) Google OAuth nếu muốn tự động upload lên Google Drive.

### Cài đặt nhanh (macOS / Linux)

```bash
git clone https://github.com/<USERNAME>/kitee-eduskill.git
cd kitee-eduskill
./setup.sh
```
...
| Biến | Ý nghĩa |
|------|---------|
| `KITEE_WORKSPACE_DIR` | Thư mục làm việc chính |
| `HERMES_EDUSKILL_OUTPUT_DIR` | Nơi lưu file `.docx`/`.pdf` |
| `EDUSKILL_DIR` | Thư mục engine eduSkill (chứa `slash.mjs`) |
...
**Không tạo được file?**
Kiểm tra `EDUSKILL_DIR` trỏ đúng thư mục chứa `slash.mjs`, và Python 3.12 có đủ `python-docx`.
