 ...
 * Ưu tiên:
 *   1. HERMES_EDUSKILL_OUTPUT_DIR / EDUSKILL_OUTPUT_DIR: đường dẫn output trực tiếp.
 *   2. HERMES_WORKSPACE_DIR / KITEE_WORKSPACE_DIR: tự lưu vào <workspace>/Output.
 *   3. Fallback legacy: <eduSkill>/output.
 */
function resolveOutputDir() {
  const direct = process.env.HERMES_EDUSKILL_OUTPUT_DIR || process.env.EDUSKILL_OUTPUT_DIR
  if (direct) return path.resolve(direct.replace(/^~(?=\/|$)/, process.env.HOME || ''))
  const workspace = process.env.HERMES_WORKSPACE_DIR || process.env.KITEE_WORKSPACE_DIR
  if (workspace) return path.join(path.resolve(workspace.replace(/^~(?=\/|$)/, process.env.HOME || '')), 'Output')
  const kiteeWorkspace = '/Users/nguyenthaivuong/Desktop/HermesWorkSpace/Kitee'
  if (fs.existsSync(kiteeWorkspace)) return path.join(kiteeWorkspace, 'Output')
